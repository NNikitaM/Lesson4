...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (String key : customersOrders.keySet()) {
            ...
            for (Double orderValue : customersOrders.get(key)) {
                currentOrderSum += orderValue;
            }
            if (currentOrderSum > maxOrder) {
                maxOrder = currentOrderSum;
                customerName = key;
            }
        }
        return customerName;
    ...
    void removeUnprofitableOrders() {
        ...
        for (String key : customersOrders.keySet()) {
            for (Double orderValue : customersOrders.get(key)) {
                ordersSum += orderValue;
            }
            if (ordersSum < 5000) {
                names.add(key);
                customerName = key;
                System.out.println("Клиента " + customerName + " больше нет в таблице.");
            }
        }
    }
}